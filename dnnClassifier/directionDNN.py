...
def train(save = True, filename = 'model_directionDNN.ckpt'):
  num_steps = 501
  with session.as_default():
    ...
    if save:
      save_path = saver.save(session, os.getcwd() + '/' + filename)

def init():
  with session.as_default():
    saver = tf.train.Saver()
    tf.train.Saver().restore(session, filedir + "/model_directionDNN.ckpt") 
